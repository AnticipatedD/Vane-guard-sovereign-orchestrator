...
import {
    PLACEHOLDER_CF_API_TOKEN,
    PLACEHOLDER_CF_AIG_TOKEN,
...
// Documentation sample only — not a real credential
export const code = `import OpenAI from "openai";

const client = new OpenAI({
    apiKey: "${PLACEHOLDER_CF_API_TOKEN}",
    {headerauth}
    baseURL:
...
const aigateway = createAiGateway({
  accountId: "${PLACEHOLDER_CLOUDFLARE_ACCOUNT_ID}",
  gateway: '${PLACEHOLDER_GATEWAY_NAME}',
  apiKey: '${PLACEHOLDER_CF_AIG_TOKEN}',
});
...
export const curlCode = `curl -X POST https://gateway.ai.cloudflare.com/v1/{account_id}/{gateway_id}/compat/chat/completions \\
  --header 'cf-aig-authorization: Bearer ${PLACEHOLDER_CF_AIG_TOKEN}' \\
{headerauth}
